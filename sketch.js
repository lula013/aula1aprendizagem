...
class Bullet {
  ...
  draw() {
    ...
    fill(red(0, 256), random(0, 256), red(0, 256)); 
...
function setup() {
  ...
  bullets = [];
  player = new Player(width/2, 0.98*height, 40);
...
class Player2 {
  ...
  move() {
    if (keyIsDown(a) && this.posX >= 5) {
      this.posX -= 15;
    }

    if (keyIsDown(d) && this.posX <= width - player.size) {
      console.log(this.posX);
      this.posX += 15;
    }

    if (keyIsDown(w) && this.posY >= 5 + player.size) {
      this.posY -= 15;
    }

    if (keyIsDown(s) && this.posY <= height - player.size) {
      this.posY += 15;
    }
  }
}
